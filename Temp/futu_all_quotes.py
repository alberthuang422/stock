...
HF = os.path.join(BASE, "Temp", "_hf_ac.txt")
...
rpc("initialize", {"protocolVersion": "2025-03-26", "capabilities": {}, "clientInfo": {"name": "ac", "version": "1"}})
rpc("notifications/initialized", {}, notify=True)

# 拉 2026-10 ~ 2027-12 共 15 个月
months = [f"{y}{m:02d}" for y in (26, 27) for m in range(1, 13)]
months = [m for m in months if m >= "2610"]
codes = [f"US.CL{m}" for m in months]

snap = tool("quote_market_snapshot", {"code_list": codes})
out = {"snapshot": {}, "kline": {}, "probe": {}}
if isinstance(snap, dict) and snap.get("data"):
    for x in snap["data"]["snapshot_list"]:
        out["snapshot"][x["code"]] = {
            "name": x.get("name"), "last": x.get("last_price"), "prev": x.get("prev_close_price"),
            "open": x.get("open_price"), "high": x.get("high_price"), "low": x.get("low_price"),
            "settle": (x.get("future_ex_data") or {}).get("last_settle_price"),
            "update": x.get("update_time"),
        }
for i, c in enumerate(codes):
    r = tool("quote_history_kline", {"symbol": c, "end": TODAY, "num": 130, "ktype": 2, "autype": 0})
    bars = []
    try:
        ...
        pass
    out["kline"][c] = bars
    print(f"{c:<12} bars={len(bars):<4} last={bars[-1]['close'] if bars else '-'}")
    time.sleep(1.2)

json.dump(out, open(os.path.join(BASE, "results", "futu_all_contracts_raw.json"), "w", encoding="utf-8"),
          ensure_ascii=False, indent=1)
print("saved -> results/futu_all_contracts_raw.json")
