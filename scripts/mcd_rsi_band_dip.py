# -*- coding: utf-8 -*-
"""
MCD RSI 区间跌落买入（阶梯式越跌越买）——用户自定义口径 2026-08-28
...
输出 results/mcd_rsi_band_dip.json
"""
...
mcd, spy = load("mcd"), load("spy")
dates = sorted(set(mcd) & set(spy))
px = [mcd[d] for d in dates]
...
res = {"meta": {"ticker": "MCD", "event": "RSI区间跌落买入：当日收盘RSI档位(35-40/30-35/<30)比前日更低位即当日收盘买入",
...
out_path = os.path.join(OUT, "mcd_rsi_band_dip.json")
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(res, f, ensure_ascii=False, indent=1)

print(f"MCD {n} 根K线 | 区间跌落事件 {len(ev)}（cd10 {len(ev_cd10)}）")
