// 原生 CDP 增量补齐：CF/DAR/MOS/TSN/HRL/XLE/SPY/CL(=F) 日线至最新
// 只追加比现有 CSV 更新日期的行，历史行不动。
const fs = require("fs");
const path = require("path");

const OUT_ROOT = path.resolve(__dirname, "../data");
const TICKERS = [
  ["CF", "CF"], ["DAR", "DAR"], ["MOS", "MOS"], ["TSN", "TSN"],
  ["HRL", "HRL"], ["XLE", "XLE"], ["SPY", "SPY"], ["CL", "CL=F"],
];
const PERIOD1 = Math.floor(new Date("2025-06-01").getTime() / 1000);
...
async function fetchChart(symbol) {
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?period1=${PERIOD1}&period2=${Math.floor(Date.now() / 1000)}&interval=1d&events=history&includeAdjustedClose=true`;
  let target = null;
  let sess = null;
  try {
...
(async () => {
  for (const [tk, symbol] of TICKERS) {
    const txt = await fetchChart(symbol);
    ...
    if (!rows.length) { console.error(tk, "empty rows"); continue; }
    const file = path.join(OUT_ROOT, tk.toLowerCase(), `${tk}, 1D.csv`);
    let existing = [];
    let lastDate = "";
    if (fs.existsSync(file)) {
      ...
      existing = ["date,open,high,low,close,volume,adj_close"];
    }
    const add = rows.filter(r => r[0] > lastDate);
    if (add.length) {
      fs.writeFileSync(file, existing.concat(add.map(r => r.join(","))).join("\n") + "\n");
    }
    console.log(`${tk}: 拉取 ${rows.length} 行 (${rows[0][0]}~${rows[rows.length - 1][0]}), 追加 ${add.length} 行${add.length ? ` (至 ${add[add.length - 1][0]})` : ""}`);
  }
  console.log("DONE");
