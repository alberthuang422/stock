# -*- coding: utf-8 -*-
"""
DAL RSI 区间跌落买入（阶梯式越跌越买）——复刻 56 号 CCL 口径 2026-09-02
...
输出 results/dal_rsi_band_dip.json
"""
...
res = {"meta": {"ticker": "DAL", "event": "RSI区间跌落买入：当日收盘RSI档位(35-40/30-35/<30)比前日更低位即当日收盘买入",
...
out_path = os.path.join(OUT, "dal_rsi_band_dip.json")
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(res, f, ensure_ascii=False, indent=1)

print(f"DAL {n} 根K线 | 区间跌落事件 {len(ev)}（cd10 {len(ev_cd10)}） | {dates[0]} ~ {dates[-1]}")
