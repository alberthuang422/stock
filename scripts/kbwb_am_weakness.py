...
输出维度：
  ...
  4) 近 3 年 / 近 1 年子样本
输出 results/kbwb_am_weakness.json；控制台打汇总 KPI。
"""
import os
import json
import math
...
def main():
    ...
    out = {
        ...
        "meta": {"kbwb": "Invesco KBW Bank ETF（银行指数代理）",
                 "techs": {"APO": "Apollo Global Management（另类资管）", "BX": "Blackstone Inc.（另类资管龙头）", "KKR": "KKR & Co.（另类资管）", "BLK": "BlackRock（传统资管龙头）", "TROW": "T. Rowe Price（传统资管）"},
    ...
    path = os.path.join(OUT, "kbwb_am_weakness.json")
