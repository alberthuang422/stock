// 渲染测试: 加载公用事业 MACD 水下金叉回测报告, 收集 JS 错误并截图
...
(async () => {
  ...
  const file = path.resolve(__dirname, "../reports/18_公用事业MACD水下金叉回测/公用事业MACD水下金叉回测报告.html");
  await page.goto("file://" + file, { waitUntil: "networkidle", timeout: 60000 });
  await page.waitForTimeout(2500);

  const chartInfo = await page.evaluate(() => {
    const ids = ["chart1", "chart2"];
    const out = {};
    for (const id of ids) {
  ...
  const dataOk = await page.evaluate(() => {
    try {
      return { tickers: D.all.tickers.length, groups: D.groups.tickers.length };
