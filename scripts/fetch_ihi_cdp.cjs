// 原生 CDP 抓取 IHI (iShares U.S. Medical Devices ETF) 的 Yahoo 日线数据
// 逻辑与 fetch_banks_cdp.cjs 相同；Chrome 须以 run_in_background=true 启动（9222 端口）
const fs = require("fs");
const path = require("path");

const OUT_ROOT = path.resolve(__dirname, "../data");
// 默认拉取 IHI/XBI；用 --tickers=A,WAT,DHR,TMO,IBB 覆盖
const argTk = process.argv.find(a => a.startsWith("--tickers="));
const TICKERS = argTk
  ? argTk.slice("--tickers=".length).split(",")
  : (process.argv.includes("--xbi") ? ["XBI"] : ["IHI"]);
...
(async () => {
  for (const tk of TICKERS) {
    ...
    if (res.error) { console.error(tk, "yahoo error:", JSON.stringify(res.error)); continue; }
