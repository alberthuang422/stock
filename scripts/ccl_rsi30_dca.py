...
for start, end in periods:
    ...
    rows.append({
        ...
        "ret_end": round(ret_end * 100, 2),                # 超卖末日结算
        "ret_t5": round(ext["ret_t5"] * 100, 2),
        "ret_t10": round(ext["ret_t10"] * 100, 2),
        "ret_t20": round(ext["ret_t20"] * 100, 2),         # 末日+T20
        "lump_end": round(lump_end * 100, 2),              # 首日一把梭@末日
        "lump_t20": round(px[end + 20] / px[start] - 1, 4) * 100,
        "spy_ret_end": round(s_ret_end * 100, 2),          # SPY 同期定投@末日
        "spy_ret_t20": round(s_ret_t20 * 100, 2),          # SPY 同期定投@T20
        "spy_lump_end": round(s_lump_end * 100, 2),
        "ex_end": round(ret_end * 100 - s_ret_end * 100, 2),        # 超额 vs SPY定投 (末日)
        "ex_t20": round(ext["ret_t20"] * 100 - s_ret_t20 * 100, 2),  # 超额 vs SPY定投 (T20)
...
out = {
    "meta": {"ticker": "CCL", "rule": "RSI14<30 期间每日收盘等额定投 $1, RSI>=30 停止; 周期=[首次<30, 末日<30]",
...
out_path = os.path.join(OUT, "ccl_rsi30_dca.json")
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)

print(f"CCL 超卖周期数: {len(rows)}  长度分布: {out['len_dist']}")
a = out["agg"]
print("\n[主口径 A: 周期末日结算]")
...
for s in ["疫情前", "疫情~2022", "本轮牛市"]:
    b = out["by_stage"][s]
    if b["n"]:
        print("  %-8s n=%2d 中位=%+.2f%% 胜率=%.1f%% 超额中=%+.2fpp (T20: %+.2f%%)" % (
            s, b["n"], b["ret_end"]["median"], b["ret_end"]["win"], b["ex_end"]["median"], b["ret_t20"]["median"]))
print("\n[长度分层 末日结算]")
