// CDP 无头校验 64 号报告：5 张 ECharts canvas、无 pageerror、关键文本存在
...
(async () => {
  ...
  const r = await sess.send("Runtime.evaluate", {
    returnByValue: true,
    expression: `(() => {
      const ids = ['ch_band','ch_cd10','ch_year','ch_deep','ch_deep2'];
      return {
  ...
  console.log('verdicts:', v.verdicts, 'tables:', v.tables);
  console.log('pageErrors:', sess.pageErrors.length);
  sess.pageErrors.forEach(e => console.log('  ERR:', e));
  await sess.send("Page.close");
  sess.close();
  const ok = v.canvas >= 5 && sess.pageErrors.length === 0 && v.perChart.every(x => x.includes('OK'));
