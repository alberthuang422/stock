...
async function main() {
  ...
  await page.screenshot({ path: path.join(__dirname, '..', 'results', 'lifetools_render_top.png'), clip: { x: 0, y: 0, width: 1400, height: 1500 } });
  await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  await page.waitForTimeout(800);
  await page.screenshot({ path: path.join(__dirname, '..', 'results', 'lifetools_render_full.png'), fullPage: true });
