// 补拉 XLY/XLB/XLC 三个板块 ETF
const fs = require("fs");
const path = require("path");
const OUT_ROOT = path.resolve(__dirname, "../data");
const TARGETS = [ { dir: "xly", q: "XLY" }, { dir: "xlb", q: "XLB" }, { dir: "xlc", q: "XLC" } ];
...
async function fetchChart(sym, attempt) {
  ...
  } catch (e) { console.error(sym, "attempt", attempt, "err:", e.message); return null; }
...
(async () => {
  ...
  for (const t of TARGETS) {
    ...
    for (let attempt = 0; attempt < 3 && !ok; attempt++) {
      ...
      if (!txt) continue;
      let j; try { j = JSON.parse(txt); } catch (e) { console.error(t.q, "attempt", attempt, "not json"); continue; }
      const res = j.chart && j.chart.result && j.chart.result[0];
      if (!res) { console.error(t.q, "no result"); continue; }
      ...
      const dir = path.join(OUT_ROOT, t.dir); fs.mkdirSync(dir, { recursive: true });
