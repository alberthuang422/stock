...
输出 results/support_ms_demo.json 供可视化；控制台只打摘要。
"""
import os
import json
...
DATA = os.path.join(ROOT, "data", "ms", "ms, 1D.csv")
...
print(f"窗口 {df['date'].iloc[0].date()} ~ {df['date'].iloc[-1].date()}  n={N}  现价={close[-1]:.2f}")
...
with open(os.path.join(OUT, "support_ms_demo.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)

print(f"\nsaved: results/support_ms_demo.json")
print(f"\n=== 支撑位 Top（近18个月）===")
for r in sup_list[:8]:
