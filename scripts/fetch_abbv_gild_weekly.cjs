// 原生 CDP 抓取 Yahoo chart JSON：ABBV / GILD 周线（interval=1w）
const fs = require("fs");
const path = require("path");

const OUT_ROOT = path.resolve(__dirname, "../data");
const TICKERS = ["ABBV", "GILD"];
...
async function fetchWeekly(ticker) {
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${ticker}?period1=${PERIOD1}&period2=${Math.floor(Date.now() / 1000)}&interval=1wk&events=history&includeAdjustedClose=true`;
  let target = null;
  let sess = null;
  try {
...
(async () => {
  for (const tk of TICKERS) {
    const txt = await fetchWeekly(tk);
    ...
    fs.writeFileSync(path.join(dir, `${tk}, 1W.csv`), csv);
