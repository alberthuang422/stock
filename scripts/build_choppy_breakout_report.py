...
<div class="tldr">
<div class="item i-red"><b>▲ 你的直觉成立：震荡市向上突破一半是假的</b>震荡窗内向上突破 10 日内跌回突破位的比例 <b>53.3%</b>，趋势日仅 44.8%；T+20 收益 +2.59% 明显弱于趋势日突破的 +5.28%，胜率 57.1% vs 66.9%。</div>
<div class="item i-green"><b>▼ 向下突破是"假摔黄金坑"：假跌破率 64.5%</b>震荡市跌破波段低点后，10 日内收复的比例 64.5%，且 T+20 平均 +2.07%、胜率 58.0%——跌破追空期望为负，跌破后回补/买入反而占优。</div>
...
<div class="note">t 值为事件横截面 t 统计量；样本按（个股×年）聚类，同窗多票非独立，显著性应视为<b>上限</b>。▲=向上突破（首破最近已确认波段高点 +0.5%），▼=向下突破。</div>
</div>
<div class="card">
...
<div class="sec-note">假突破定义：▲突破后 T+10 内任一收盘 < 突破位；▼突破后 T+10 内任一收盘 > 突破位（收复）。</div>
<div class="card">
...
<tr><td class="l">突破事件</td><td class="l">收盘 > 最近已确认波段高点 ×1.005 记▲；< 波段低点 ×0.995 记▼；首破计数（前一日不在同向突破态），同向 20 交易日冷却</td></tr>
...
(function(){
  ...
  const name = {up_choppy:'▲ 突破 ∈ 震荡窗',up_trend:'▲ 突破 ∈ 趋势日',dn_choppy:'▼ 跌破 ∈ 震荡窗',dn_trend:'▼ 跌破 ∈ 趋势日',baseline_choppy:'○ 震荡日随机基线',baseline_trend:'○ 趋势日随机基线'};
...
(function(){
  ...
  const rows = [
    ['全样本 ▲∈震荡窗', 623, '+2.59%','57.1%','+1.78% <span class="mut">(t=3.02)</span>','53.3%','主口径 1993-2026'],
    ['蓝筹 ▲∈震荡窗 2010后', 51, '+0.45%','58.8%','<span class="neg">−0.88%</span>','—','<b>无可操作 edge</b>'],
    ['热票 ▲∈震荡窗 2015后', 197, '+3.49%','51.8%','+2.37% <span class="mut">(t=1.61 未显著)</span>','58.9%','新浪未复权，动量票'],
    ['路径A纯口径 ▲', 108, '+3.16%','61.1%','+2.68% <span class="mut">(t=1.85)</span>','46.3%','仅均线缠绕激活日'],
    ['K=8% 敏感性 ▲∈震荡窗', 644, '+1.75%','54.0%','+1.12%','52.8%','定性结论不变'],
  ];
...
(function(){
  ...
  const rows = [
    ['9-10月 ▲∈震荡窗', 137, '+2.18%','57.7%','+1.01%','0.79','弱于其他月份(+1.99% t=3.00)'],
    ['9-10月 ▼∈震荡窗', 604, '+2.59%','61.9%','+0.95%','2.33','胜率略高于其他月(57.0%)'],
    ['中期选举前2月 ▲', 9, '−4.33%','33.3%','−2.66%','−0.61','n 过小仅参考'],
    ['中期选举前2月 ▼', 87, '−4.25%','24.1%','−2.10%','−1.95','<b>全线翻车，跌破也不反弹</b>'],
    ['大选前2月 ▲', 8, '+8.29%','75.0%','+3.03% <span class="mut">(合并t=2.25)</span>','—','n=8 轶事级'],
    ['大选前2月 ▼', 53, '+6.04%','79.2%','—','—','跌破后大反弹（2020 ▼+5.31%/82.8%；2024 ▼+7.49%/75.0%）'],
  ];
  rows.forEach(r=>{ h += `<tr><td class="l">${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td><td>${r[4]}</td><td>${r[5]}</td><td class="l mut" style="white-space:normal">${r[6]}</td></tr>`; });
...
(function(){
  ...
  const rows = [
    ['主口径 ▲∈震荡窗', m['up_choppy']],
    ['路径A纯口径 ▲', m['up_choppyA_only']],
    ['主口径 ▼∈震荡窗', m['dn_choppy']],
    ['路径A纯口径 ▼', m['dn_choppyA_only']],
  ];
  rows.forEach(([l, r])=>{
    if(!r) return;
...
(function(){
  const cats = ['▲∈震荡窗','▲∈趋势日','▼∈震荡窗','▼∈趋势日','○震荡日基线','○趋势日基线'];
