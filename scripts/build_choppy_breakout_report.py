...
<div class="tldr">
<div class="item i-red"><b>向上突破：突破后不回踩的才不足一半</b>震荡窗内向上突破后 T+10 完全不回踩的仅 <b>43.9%</b>（这部分胜率 69.9%）；回踩深度与 T+20 收益单调负相关，≥1.5 ADR 确认假突破后胜率仅 25.8%。震荡市 T+20 +2.59% 弱于趋势日 +5.28%。</div>
<div class="item i-green"><b>向下突破："假摔黄金坑"</b>震荡市跌破波段低点后，T+20 平均 +2.07%、胜率 58.0%；若 10 日内反向偏离 ≥1.5 ADR（假摔），后续 T+20 中位 <b>+4.15%</b>——跌破追空期望为负，假摔后买入弹性更大。</div>
...
<div class="note">t 值为事件横截面 t 统计量；样本按（个股×年）聚类，同窗多票非独立，显著性应视为<b>上限</b>。假突破率=T+10 内价格偏离突破位 ≥1.5×ADR（20 日）的比例。"向上突破"=收盘 > 最近已确认波段高点×1.005；"向下突破"=收盘 < 最近已确认波段低点×0.995。</div>
</div>
<div class="card">
...
<div class="note"><b>假突破口径（修订）</b>：T+10 内价格偏离突破位 ≥1.5×ADR（20 日平均真实波幅%）——只把实质性反转算假突破，贴着突破位的正常噪音不算。解读：<b>①</b> 新口径下假突破率整体降到 13-22%，但排序不变：震荡窗向上突破 12.8% 仍高于趋势日 9.6%，向下突破假摔率 20%+ 高于向上；<b>②</b> 与胜率不矛盾——假突破（碰线）与 T+20 盈亏是两个独立度量的窗口，向下突破假摔后常伴随深弹反弹（假摔事件 T+20 中位 +4.15%），故胜率仍可过半；<b>③</b> 真突破（10 日内未偏离 1.5 ADR）的 T+20 胜率：向上 53.6%/60.7%、向下 50.4%，明显优于假突破事件——"等 1.5 ADR 确认"有区分价值。</div>
</div>

<div class="card">
<div class="sec-note" style="margin-bottom:8px;">把"真/假"二元标签拆开——T+10 内最大反向偏离（按 ADR 倍数）分五层，收益随回踩深度<b>单调恶化（向上）/单调改善（向下）</b>，说明突破质量是连续谱、不是二值事件：</div>
<table id="t_layer"></table>
...
<div class="note"><b>⚠ 关键对照——"MFE &gt; MAE"不等于可以追突破</b>：随机日基线（同批股票、随机日期买入）的 T+20 MFE/MAE 中位为 +5.04% / −3.16%，与向上突破的 +5.84% / −3.97% 几乎一样。差异：突破日的 MFE-|MAE| 差值 +1.87pp vs 随机日 +1.88pp（<b>无优势</b>），且突破日较差情形（MAE 25 分位）−10.3% <b>深于</b>随机日 −7.67%。这个"MFE 优于 MAE"的表象来自美股长期正漂移+波动率的不对称性，是任何日期买入都白送的，不是突破信号的价值。<b>真正有差异的是向下假摔：反向偏离 ≥1.5 ADR 后买入的 T+20 收益中位 +9.19%（vs 随机日 +1.88%）</b>——可追的是"确认后的假摔反弹"，不是突破本身。另：震荡市突破适合"小止盈、严止损"（T+20 持有中位 +2.59% vs 期间浮盈机会 +5.84%，过半浮盈会回吐）；向下突破 T+60 浮亏中位 −11.72%，假摔买入必须配更宽止损或更小仓位。</div>
</div>
...
<tr><td class="l">延续性指标</td><td class="l">T+5/10/20/60 持有与超额收益、T+10 假突破率（偏离突破位 ≥1.5×ADR(20日)，用户修订口径）、T+20 存活率（最大不利偏移 > −5% 为存活）、MAE 中位数</td></tr>
...
(function(){
  ...
  h += '<tr><th class="l"></th><th>占比</th><th>T+20胜率</th><th>T+20中位</th><th>占比</th><th>T+20胜率</th><th>T+20中位</th></tr>';
  const rows = [
    ['未回踩/回补（无反向偏离）', '43.9%','69.9%','+4.82%','35.5%','35.4%','−2.11%'],
    ['0 ~ 0.5 ADR', '20.0%','56.5%','+1.24%','18.6%','54.1%','+0.99%'],
    ['0.5 ~ 1 ADR', '12.7%','40.7%','−0.76%','15.7%','66.9%','+2.99%'],
    ['1 ~ 1.5 ADR', '7.8%','27.3%','−5.58%','9.9%','73.5%','+4.19%'],
    ['≥1.5 ADR（假突破/假摔）', '15.6%','25.8%','−7.52%','20.3%','86.6%','+9.19%'],
  ];
  rows.forEach(r=>{
    h += `<tr><td class="l">${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td><td>${r[4]}</td><td>${r[5]}</td><td>${r[6]}</td></tr>`;
  });
  document.getElementById('t_layer').innerHTML = h;
})();
