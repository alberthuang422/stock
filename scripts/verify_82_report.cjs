// CDP 无头校验 82 号报告：canvas=4、无 pageerror、术语浮窗可触发
...
(async () => {
  ...
  const r = await sess.send("Runtime.evaluate", {
    ...
    expression: `(() => ({
      ...
      chartsInit: ['c_prod','c_struct','c_t5','c_su','c_bal','c_px'].map(id => !!(document.getElementById(id) && document.getElementById(id).querySelector('canvas'))),
      fullTableRows: document.getElementById('t_full') ? document.getElementById('t_full').rows.length : 0,
      errs: window.__errs || []
    }))()`
  });
