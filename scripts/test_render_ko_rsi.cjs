// SSR 渲染测试：ko_rsi_overbought_report.html — 验证图表 canvas 与表格行数
...
(async () => {
  ...
  const file = "file:///" + path.resolve(__dirname, "../reports/17_ko_rsi_overbought/ko_rsi_overbought_report.html").replace(/\\/g, "/");
  ...
  const info = await page.evaluate(() => {
    ...
    return { totalCanvas: canvases.length, zeroSize: zero, tbodies,
             dataLen: typeof DATA !== "undefined" ? (DATA.dailyD ? DATA.dailyD.length : "no dailyD") : "DATA undefined" };
  });
