// IHI×XBI 相关性报告 SSR 渲染测试
// 用法: NODE_PATH=<workspace node_modules> node scripts/test_render_ihi_xbi.cjs
const fs = require('fs');
const path = require('path');
const { chromium } = require('playwright-core');

const CHROME = 'C:/Users/Administrator/AppData/Local/Google/Chrome/Application/chrome.exe';
const htmlPath = path.join(__dirname, '..', 'reports', '23_ihi_xbi器械vs生物科技', 'index.html');
...
async function main() {
  ...
  const needTxt = ['核心结论', '分阶段相关性一览', '年度相关性', '60 日滚动相关性', '月度相关性',
                   '日收益散点', '极端日分析', 'Fisher', '0.249', '0.666', '6.34', '195', '46.2',
                   '结构断裂点', '2026-02'];
  ...
  if (Object.keys(chartInfo).length < 5) errors.push('图表数量不足 5');
  ...
  // 截图
  await page.screenshot({ path: path.join(__dirname, '..', 'results', 'ihi_xbi_render_top.png'), clip: { x: 0, y: 0, width: 1400, height: 1500 } });
  await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  await page.waitForTimeout(800);
  await page.screenshot({ path: path.join(__dirname, '..', 'results', 'ihi_xbi_render_full.png'), fullPage: true });
