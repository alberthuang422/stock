// 抓取 blue_chips.csv 缺失的蓝筹股 Yahoo 日线（原生 CDP WebSocket，模板取自 fetch_banks_cdp.cjs）
// 2026-08-27 已拉齐 44/45 只；MMC 因 Yahoo chart 接口持续返回 "symbol may be delisted" 暂缺
const fs = require("fs");
const path = require("path");

const OUT_ROOT = path.resolve(__dirname, "../data");
const TICKERS = [
  "GOOGL","TXN","AVGO","ORCL","QCOM","ACN",
  "DE","ETN","PH","ROK","TDG","UNP","FDX","UPS",
  "NVO","ISRG","MCK","ABT","BDX","SYK",
  "LOW","TJX","BKNG","CMG","ORLY","MAR","CPRT",
  "PCAR","WM","RSG","CL","KMB","GIS","HSY","MDLZ","COST",
  "LMT","SPGI","ICE","AON","MMC","AJG","SO","DUK","AWK",
];
...
async function cdpSession(wsUrl) {
  ...
  return { openP, send, close: () => { try { ws.close(); } catch (e) {} } };
}
...
async function fetchChart(ticker) {
  ...
  try {
    ...
    if (sess) sess.close();
    if (target && target.id) { try { await fetch(`${CDP}/json/close/${target.id}`); } catch (e) {} }
  }
}
...
(async () => {
  ...
  for (const tk of TICKERS) {
    ...
    let j;
    try { j = JSON.parse(txt); } catch (e) { console.error(tk, "not json, head:", txt.slice(0, 120)); failed++; continue; }
    const res = j.chart && j.chart.result && j.chart.result[0];
    if (!res) { console.error(tk, "no result:", JSON.stringify(j).slice(0, 120)); failed++; continue; }
    ...
    if (!rows.length) { console.error(tk, "empty rows"); failed++; continue; }
    const dir = path.join(OUT_ROOT, tk.toLowerCase());
    fs.mkdirSync(dir, { recursive: true });
    const csv = ["date,open,high,low,close,volume,adj_close", ...rows.map(r => r.join(","))].join("\n");
