...
8 标的：CSCO/MCD/VST/APO（波段多）+ ABBV/GILD（核心多）+ SBUX/XYZ（空仓观望）
...
PLANS = [
    ...
    {
        "tk": "SBUX", "name": "星巴克", "dir": "观望", "ptype": "—",
        "logic": "多头（> EMA20 106.3 > EMA50 104.9 > EMA200 98.9）但逼近前高 110.5 且 8/26 冲 108.6 回落，RSI 53.9 中性。当前位置（107.3）既不处于 103–104 回调低吸区、也未放量上破 110.5——缺乏高值博率买点。51 号报告：SBUX 2026 年以来 +25.9% 独立反弹，波动率（年化 32%）高，追高成本大。",
        "supports": [("103–104", "EMA50/前低"), ("101.8", "8/4 低")],
        "resists": [("110.5", "8/13 高")],
        "sl": "—", "targets": [("突破 110.5 后", "看新高"), ("回踩 103–104", "低吸")],
        "pos": "空仓观望：回调 103–104 或放量突破 110.5 二者出现其一再建仓",
    },
    {
        "tk": "XYZ", "name": "环球音乐", "dir": "观望", "ptype": "—",
        "logic": "多头（> EMA20 81.4 +4.3% > EMA50 79.0 > EMA200 72.1），8/27 放量 +2.11%（量 576 万 vs 20日均 324 万）上攻，逼近 253 日高 86.9。RSI 61 转强但未超买。追高区间 84.9→86.9 仅剩 +2.4% 空间、上方无历史筹码参照，值博率一般；若突破 86.9 放量则打开 90+ 空间。",
        "supports": [("81.4", "EMA20"), ("79.0", "EMA50")],
        "resists": [("86.9", "8/5 高·253日高")],
        "sl": "—", "targets": [("突破 86.9 后", "看 90+"), ("回踩 81–82", "低吸")],
        "pos": "空仓观望：优先等回踩 81–82，突破 86.9 再考虑跟进（带量确认）",
    },
]
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .dir-观望 {{ color: var(--grey); font-weight: 700; }}
...
<div class="wrap">
  ...
  <div class="subtitle">8 标的逐一个股拆解（核心 2 + 波段 4 + 观望 2） · 宏观前提：通胀粘滞 + 10Y-2Y 利差扩大 · 市场结构：指数横盘 + 板块快速轮动 · 数据截至 2026-08-27（周四）收盘 · 生成于 2026-08-28</div>
  ...
  <div class="card">
    ...
    <ul class="tl">
      ...
      <li><b>风险 5 · 空仓标的踏空 vs 追高</b>：SBUX/XYZ 已给出建仓信号前继续观望，但 XYZ 若放量突破 86.9、SBUX 若突破 110.5，属"趋势启动而非追高"，可各以 1/3 试探仓跟进（止损分别 81 / 103）。</li>
    </ul>
    <div class="risk">
      <b>总体判断</b>：组合结构上"核心（ABBV/GILD）健康可持有、波段（CSCO/MCD/VST/APO）需精挑时点、观望（SBUX/XYZ）等信号"。在指数横盘+板块轮动的环境下，<b>核心仓位纪律 &gt; 波段进攻</b>：建议维持核心 55–60%、波段 ≤25%、现金 ≥15% 的配置，波段仓全部以收盘止损为硬约束，MCD 加仓等待 RSI≤35 或止跌确认。
    </div>
