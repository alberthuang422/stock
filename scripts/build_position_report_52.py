...
PLANS = [
    ...
    },
    {
        ...
        "logic": "空头逆势仓位：现价 107.3 仍站上全部均线（多头排列），逼近 8/27 高点 108.6 与前高 110.5 压力区，8/26 冲高 108.6 后连续两日回落（108.49→107.26），RSI 53.9 中性回落。空头逻辑：①前高 110.5 为 253 日区间上沿，未放量突破前赌反弹衰竭；②51 号报告：SBUX 波动率年化 32% 高，空头须窄止损；③板块：XLY 近 1 月 −1.0% 震荡、8/27 防御/消费集体被抽血。⚠️ 逆势空：无趋势支持，本质是'前高压力位赌回落'的区间空，不是趋势空。",
        "supports": [("106.3", "EMA20·第一目标"), ("104.9", "EMA50·第二目标"), ("103–104", "前低带")],
        "resists": [("108.6", "8/27 高·止损触发"), ("110.5", "8/13 高·硬止损")],
        "sl": "收盘 > 110.5（站上前高=逼空，无条件回补）", "targets": [("104.9", "−2.2%"), ("103", "−4.0%")],
        "pos": "空头持有：不加空、不扛单；收盘突破 110.5 认输回补；目标 104.9 减半、103 平仓",
    },
    {
        "tk": "XYZ", "name": "环球音乐", "dir": "空", "ptype": "空头",
        "logic": "空头逆势仓位，正处组合内最危险位置：8/27 放量 +2.11%（量 576 万 vs 20 日均 324 万）直逼 253 日高 86.9，现价距前高仅 +2.4%。空头逻辑：①86.9 为 253 日区间上沿强阻力，未有效突破则假突破回落；②RSI 61 连续上行后追多动能见顶风险；③50 号报告：XYZ 与 BTC 联动弱（r≈0.28），近波上涨非 BTC 驱动、持续性存疑。⚠️ 组合内逼空风险最高的一腿：多头排列+放量逼近新高，突破 86.9 的可能性真实存在，必须预设无条件回补线。",
        "supports": [("81.4", "EMA20·第一目标"), ("79–80", "EMA50/前低带"), ("77.2", "8/12 低·深度目标")],
        "resists": [("86.9", "253日高·止损触发"), ("88+", "突破确认·硬止损")],
        "sl": "收盘 > 86.9（放量站上前高=突破成立，无条件回补）", "targets": [("81.4", "−4.1%"), ("79", "−6.9%")],
        "pos": "空头持有：已贴身止损触发区，86.9 被放量突破立即回补；目标 81.4 减半平仓",
    },
]
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <ul class="tl">
      ...
      <li><b>风险 5 · 空头腿的逼空风险（组合最大尾部风险）</b>：SBUX/XYZ 均为<b>逆势空头</b>——现价都在多头排列且贴近 253 日高（XYZ 距 86.9 仅 +2.4%、8/27 放量 +2.11%），做空与趋势反向，一旦放量突破（XYZ>86.9 / SBUX>110.5）即逼空，亏损无上限。<b>硬约束：两腿均预设无条件回补线，突破即平，绝不扛单。</b></li>
    </ul>
    <div class="risk">
      <b>总体判断</b>：组合结构上"核心多（ABBV/GILD）健康可持有、波段多（CSCO/MCD/VST/APO）需精挑时点、空头腿（SBUX/XYZ）逆势且贴逼空区"。在指数横盘+板块轮动的环境下，<b>核心仓位纪律 &gt; 波段进攻</b>：建议维持核心 55–60%、波段 ≤25%、现金 ≥15% 的配置，波段仓全部以收盘止损为硬约束，MCD 加仓等待 RSI≤35 或止跌确认。
    </div>
