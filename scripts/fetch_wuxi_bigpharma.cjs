...
// 药明康德(H/A) + 美国大药企 ABBV/MRK/LLY（JNJ/GILD 已有）
const { chromium } = require("playwright-core");
const fs = require("fs");
const path = require("path");

const OUT_ROOT = path.resolve(__dirname, "../data");
const TICKERS = ["2359.HK", "603259.SS", "ABBV", "MRK", "LLY", "JNJ", "SPY"];
...
(async () => {
  ...
  for (const tk of TICKERS) {
    ...
    console.log(`${tk}: ${rows.length} 行, ${rows[0][0]} ~ ${rows[rows.length-1][0]} 已保存 -> ${dir}`);
  }
  await browser.close();
