// 通过本机 Chrome CDP 抓取 Yahoo chart API: ^VIX (CBOE 恐慌指数) + ^GSPC (标普500指数)
const { chromium } = require("playwright-core");
const fs = require("fs");
const path = require("path");

const OUT_ROOT = path.resolve(__dirname, "../data");
// VIX 自 1990-01-02 起有数据；GSPC 取同窗口以对齐
const PERIOD1 = Math.floor(new Date("1990-01-01").getTime() / 1000);
const TICKERS = ["^VIX", "^GSPC"];
const DIRS = { "^VIX": "vix", "^GSPC": "gspc" };
const FILES = { "^VIX": "VIX, 1D.csv", "^GSPC": "GSPC, 1D.csv" };
...
(async () => {
  ...
  for (const tk of TICKERS) {
    ...
    for (let i = 0; i < ts.length; i++) {
      ...
      rows.push([dateStr, q.open[i], q.high[i], q.low[i], q.close[i], q.volume[i]]);
    }
    const dir = path.join(OUT_ROOT, DIRS[tk]);
    fs.mkdirSync(dir, { recursive: true });
    const csv = ["date,open,high,low,close,volume",
      ...rows.map(r => r.join(","))].join("\n");
    fs.writeFileSync(path.join(dir, FILES[tk]), csv);
    console.log(`${tk}: ${rows.length} 行, ${rows[0][0]} ~ ${rows[rows.length - 1][0]} 已保存 -> data/${DIRS[tk]}/${FILES[tk]}`);
  }
  await browser.close();
