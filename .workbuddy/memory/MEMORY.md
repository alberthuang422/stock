...
- **交付必须产物展示（08-29 晚设定，替代此前"不打开预览"旧规则）**：报告/网页类交付完成后**必须调用 present_files 在产物面板展示并自动打开预览**（用户可见报告卡片+右侧内置浏览器预览）；渲染验证仍做 CDP 无头检查（图表 canvas 数 + 无 pageerror）作为兜底，但展示不可省。用户原话「以后按照这种方式来」。
- **术语一律用悬停浮窗，不用速查表（08-29 设定）**：研报中**所有专业术语**（含不常见的，如 onset/SST/ENSO/β₁₀/双因子回归）写正文时**必须**虚线下划线 + 鼠标悬停弹白话解释；**禁止再放"术语速查表"区块**（用户原话"把用户当白痴，不常见术语都要悬浮"——标注偏过不偏漏）。实现模板：build 脚本里 `TERMS` 词典 + `annotate_terms()` 段落式标注（跳过 script/style，`_BLOCK_RE` 必须带捕获组否则 re.split 丢 JS 块）+ `.term`/`.termtip` CSS + 浮窗引擎 JS；标注意按 `_TAG_SPLIT_RE` 分片只标文本节点防标签属性误标。
